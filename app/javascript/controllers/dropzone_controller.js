...
import { tiffToPNG } from 'utils/tiff'
...
class DropzoneController extends Controller {
  ...
  bindEvents () {
    this.dropzone.on('addedfile', file => setTimeout(() => this.handleFileAdded(file), 500))
    this.dropzone.on('removedfile', this.handleFileRemoved)
    this.dropzone.on('success', this.handleFileSuccess)
    this.dropzone.on('queuecomplete', this.handleQueueComplete)
    this.dropzone.on('drop', this.handleFileDropped)
    this.dropzone.on('uploadprogress', this.handleFileProgress)
    this.dropzone.on('thumbnail', this.handleThumbnail)
    this.dropzone.on('error', this.handleFailedUpload)
  }
  ...
  handleFailedUpload = (file, _error) => {
    const errorTemplate = this.element.querySelector('template#uploadError').content.cloneNode(true).querySelector('div')
    errorTemplate.prepend(file.name)
    errorTemplate.append(this.dropzone.options.acceptedFiles.replace(',', ', '))

    file.previewTemplate.replaceWith(errorTemplate)
  }
...
class DirectUploadController {
  constructor (controller, file, input) {
    const { directUploadUrl: url } = input.dataset
