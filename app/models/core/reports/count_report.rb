...
class Core::Reports::CountReport
  ...
  def run
    counts = Core::CountStatusLog.completed.where(created: start_at..end_at).group(:user_id).count
    counts = counts.map do |user_id, count|
